...
int ami_write_png(char name[200], unsigned char *red, unsigned char *green, unsigned char *blue, int width, int height)
{
  ...
  nx=width;
  ny=height;
  data=(unsigned char *) malloc(sizeof(unsigned char)*size*nc);
  for(i=0;i<height;i++){
    for(j=0;j<width;j++){
      k=i*width+j;
      k2=(height-1-i)*width+j;
      data[k2]=red[k];
      data[k2+size]=green[k];
      data[k2+2*size]=blue[k];
    }
  }
  k=io_png_write_u8(name,data,nx,ny,nc);
  if(k!=0){
    printf("file %s can not be written\n",name);
    free(data);
    int val = scanf("%d",&size);
		if(val==EOF)
...
int ami_read_png(char name[200], unsigned char **red, unsigned char **green, unsigned char **blue, int *width, int *height)
{
  ...
  size_t nxp,nyp;
  int i,j,k,k2,size;
  data=io_png_read_u8_rgb(name, &nxp, &nyp);
  if(data==NULL){
    printf("file %s can not be readed\n",name);
    int val = scanf("%d",&size);
		if(val==EOF)
			printf("Error reading size\n");
    return(-1);
